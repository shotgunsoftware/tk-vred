...
class PrePublishHook(Hook):
    ...
    def execute(self, tasks, work_template, progress_cb, user_data, **kwargs):
        ...
        """
        self.parent.engine.log_info("Starting secondary pre-publish")
        
        prevNamesList = []
        dubNamesList = []
        
        for task in tasks:
            if task["item"]["name"] in prevNamesList:
                dubNamesList.append(task["item"]["name"])
            prevNamesList.append(task["item"]["name"])
                
        dubErrorList = []
        for dubName in dubNamesList:
            for task in tasks:
                if task["item"]["name"] == dubName:
                    dubErrorList.append({
                            "task":task,
                            "errors": ["Duplicate Node Name Found."]
                        })
