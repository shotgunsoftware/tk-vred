...
class VREDStartVersionControlPlugin(HookBaseClass):
    """
    Simple plugin to insert a version number into the VRED file path if one
    does not exist.
    """
    ...
    def accept(self, settings, item):
        ...
        if path:
            ...
            if version_number is not None:
                self.logger.info(
                    "VRED '%s' plugin rejected the current session..." %
                    (self.name,)
                )
                self.logger.info(
        ...
        self.logger.info(
            "VRED '%s' plugin accepted the current session." %
            (self.name,),
