...
class PrimaryPublishHook(Hook):
    ...
    def execute(
        ...
        :raises:                Hook should raise a TankError if publish of the
                                primary task fails
        """
        self.parent.engine.log_info("Starting primary publish")
        # get scene path
        scene_path = self.parent.engine.get_current_file()

        self.parent.engine.log_info("Template: {}".format(str(work_template)))
        ...
        self.parent.engine.save_current_file(scene_path)
